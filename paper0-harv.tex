...
Decisions across these levels are inherently interconnected~\citep{Melo2025}. 
For instance,~\citet{Ma2013Mult} propose a multilevel framework that jointly optimises strategic admission decisions and the tactical allocation of surgical resources. 
Using a Mixed Integer Programming (MIP) formulation, they determine the optimal CMP defined as the number of patients from each pathology group to admit over a planning horizon, maximizing hospital profit subject to resource bottlenecks such as OT time and bed capacity. 
...
Their model assumes deterministic surgery durations, thereby overlooking potential overtime and cancellations that propagate downstream and influence bed occupancy --- i.e., their MSS objective function would be impacted by these factors.
Moreover, in contrast to methods focused on queue dynamics~\citep{Siqueira2018INTO, Kayvanfar2025}, the overall objective is strictly financial, and waiting-list balancing is not considered, limiting the model's ability to address long-term patient-flow management.

Similarly,~\citet{Fugener2015} integrates strategic and tactical decisions with the aim of maximising hospital revenues. 
He formulates the problem as a MIP model supported by a stochastic patient-flow model that captures variability in postoperative resource use. 
The focus, however, lies primarily on ward-bed demand variability, so stochasticity is incorporated only at this downstream stage. 
Yet, uncertainty in surgery durations should also be considered, as it directly affects operating-room utilisation, subsequent ward occupancy, and ultimately the associated costs included in the objective function they propose.

% ---- o artigo do Liu pode se encaixar aqui porque ele fala da importandia do tamanho da fila
Also building on the need to integrate decision levels,~\citet{Liu2019} address an integrated capacity planning and scheduling problem in a two-stage hospital system.
Their goal is to determine dynamic daily admission levels for elective patients so as to minimize long-run expected costs, balancing waitlist delays against idling and overtime costs at both stages. 
The authors formulate the problem as a Markov Decision Process (MDP) and develop an analytically tractable transformation that reveals structural properties that characterize the optimal admission policy. 
% TODO: DETALHAR UM POUCO MAIS COMO TRATAM AS FILAS
Although comprehensive in modeling patient flow and stage interdependencies, the approach abstracts important real-world constraints, such as surgeon availability, case-mix heterogeneity, block scheduling structures, or hard capacity limits.
Thus, despite offering valuable insights on how waitlist size should guide admission decisions in an integrated framework, the model does not capture key sources of variability that arise in realistic strategic-tactical integration settings.

% ---- o artigo do McRae pode se encaixar por analisar niveis de agregação no case-mix
\citet{Mcrae2020} examine the strategic CMP problem and assess how uncertainty and the level of aggregation influence long-term capacity-allocation decisions. 
CMP is linked to the MSS problem through one of the authors' proposed extensions: when this extension is enabled, CMP and MSS are solved jointly; otherwise, they are addressed sequentially. 
...
Nonetheless, uncertainty is incorporated via Sample Average Approximation (SAA), which converts the stochastic programme into a deterministic approximation and may therefore fail to reflect the full underlying distributions. 
In addition, the framework does not provide the possibility of optimise waiting times of patients, which is crucial for equitable healthcare delivery.

% ---- (verificar) artigo do Samudra pode ser citado aqui também
% ---- artigo do Zhu também
% ---- FINALZIA A IDEIA LIGANDO COM O NOSSO TRABALHO: "
% Other similar integrations include X, Y, Z.
% Though, the above works lack an even more realistic treatment of uncertainty, so that the proposed models match the number of resources at the strategic level with the actual demand and surgery durations observed at the tactical level in a real-world hospital setting.
