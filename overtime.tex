...
This estimation determines whether the batch sizes $\upperq$ and $\lowerq$, for each specialty $\specialtyidx \in \specialtyset$, defined at the strategic level can be effectively served within the planning horizon.
...
\begin{table}
    \centering
    \caption{Table of Symbols for Overtime and cancellation management process.\label{tab:symb_newsvendor}\textcolor{red}{notaçao entre seçoes está caótica, vou sincronizar tudo no symb.tex}.}
    \scriptsize
    \begin{tabular}{cl}
    \toprule
...
% ******************************************************************************************** %
Building on the theoretical results of the previous subsection, we evaluate, for a block of length $\blocklength \in \mathbb{Z}_{+}$, the probability that a further surgery can be accommodated after $\completedsurgsidx \in \mathbb{N}$ surgeries have been completed. 
...
Let $\duration_{\surgidx}$ denote a non-negative the random variable representing the duration of the $i$-th surgery, with probability distribution $f_{\duration_{\surgidx}} : \Omega \to [0,1),\, \Omega \subset \mathbb{R}_{+}$ and cumulative distribution function $F_{\duration_{\surgidx}} : \Omega \to [0,1)$.
Let $s_k = \sum_{i=1}^{\completedsurgsidx} \duration_{\surgidx}$ denote the cumulative duration of the first $k$ surgeries. 
...
\end{align*}
where $\cumulativedurationcdf : \Omega \to [0,1),\, \Omega \subset \mathbb{R}_{+}$ is the cumulative distribution function of $s_k$.

We also define the random variable $\numcompletedsurgs$ as the non-negative number of surgeries that can be executed within the block, with probability distribution $f_{\numcompletedsurgs} : \Omega \to [0,1),\, \Omega \subset \mathbb{N}_{+}$ and cumulative distribution function $F_{\numcompletedsurgs} : \Omega \to [0,1)$.
Therefore, the probability that more than $k$ surgeries can be performed is given by
\[
P(\numcompletedsurgs > k) = P(s_k < \blocklength - \safetythreshold).
\]

\textcolor{red}{Preciso rever a explicacao do criterio de parada, porque do jeito em que está, eu explico o $K_{max}$ mas nem uso ele, só a ideia... Rever a explicação e o pseudocódigo tbm.}
...
\]
where $\probstoppingthreshold \in (0,1)$ is a pre-specified probability threshold. 
This definition avoids ambiguity by directly linking $K_{\max}$ to the stopping criterion used in the algorithm.

Given the model parameters $\blocklength, \safetythreshold, \probstoppingthreshold$, and the discretised probability mass function of surgery durations $\durationpmf_{\specialtyidx}$ for all $\specialtyidx \in \specialtyset$, we compute, for each subspecialty, the probabilities $P(\numcompletedsurgs > k)$ for $k = 0, \ldots, K_{\max}$ iteratively using convolution operations, as described in Algorithm~\ref{alg:newsvendor}.

\begin{algorithm}[H]
\caption{Computation of $P(\numcompletedsurgs > k)$ via convolutions and the newsvendor model\label{alg:newsvendor}}
\begin{algorithmic}[1]
...
\For {each $\specialtyidx \in \specialtyset$}
    ...
    \While{$\survivalvector[\completedsurgsidx] > \probstoppingthreshold$}
        \If{$k > 0$}
            \State $c^{(k)} \gets \text{Convolve}(c^{(\completedsurgsidx-1)}, \durationpmf_{\specialtyidx})$
        \EndIf
...
In Algorithm~\ref{alg:newsvendor}, the function \texttt{Convolve} computes the distribution of the cumulative duration $s_k$ by convolving the distribution of $s_{\completedsurgsidx-1}$ with the single-surgery duration distribution $\durationpmf_{\specialtyidx}$. 
