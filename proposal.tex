...
The queue management module establishes targets for the number of surgeries to be performed at each planning period (e.g., monthly). Specifically, a modified $(R,Q)$ policy defines, for each medical sub-speciality, a lower and an upper bound on the number of patients to be admitted for surgery during the planning period, while ensuring that system-wide waiting-time requirements are satisfied in spite of the uncertainty in patient arrivals.% This module supports the long-term balance of the waiting lists by aligning surgery admission with periodic patient arrivals via a classical inventory model under uncertainty.

%based on demand characteristics and waiting-time requirements. Specifically, a modified $(R,Q)$ inventory policy uses the probability distribution of patient demand for each medical subspecialty to define lower and upper bounds on the number of patients to be admitted each month, while ensuring that waiting-time requirements are satisfied at a given confidence level.  This module supports the long-term balance of the waiting list by aligning admission decisions with demand uncertainty and institutional access targets.
...
%The second module addresses overtime and cancellation management by evaluating the trade-off between overtime, idle time, and the risk of procedure cancellation under stochastic surgery durations.  Essentially, a Newsvendor-based model is used to determine an optimal time for the allocation of the final surgery in a block, which is then translated into a target number of surgeries to be planned in each block, balancing the costs of overtime and idle time against the risk of exceeding block capacity. 

The outputs of the first two modules provide are then combined to provide key inputs to the third module, which formulates the OT scheduling problem. Specifically, given the probability distribution of the number of performed surgeries per session (module 2), and the bounds on the number of surgeries to be performed in the period (module 1), one can derive lower and upper limits on the number of blocks to be allocated to each surgical sub-speciality. Hence, theatre allocation decisions are informed not only by resource availability and clinical compatibility, but also include constraints to guarantee that waiting-time requirements can be met despite the uncertainties in the demand and duration of surgeries. Crucially, the scheduling model remains deterministic, thereby facilitating practical implementation and ensuring rapid solution times.

%The outputs of these two modules provide key inputs to the third module, which formulates the OT scheduling problem.  In this way, monthly theatre allocation decisions are informed not only by resource availability and clinical compatibility, but also by demand uncertainty and block-level execution risk. At the same time, the scheduling model remains deterministic, which facilitates solution within practical computational times for real-world implementation.
