...
The framework captures dynamic surgical demand through explicit waiting-list modelling, enabling the enforcement of waiting-time requirements and long-term queue stability, and incorporates stochastic surgery durations to manage overtime, idle time and cancellation risk. 
The proposed approach is grounded in a real-world application at the Clinical Hospital of the University of Campinas (Brazil), a high-complexity public hospital that provides detailed operational data and a challenging planning environment~\citep{hc_unicamp_historia}.
...
\begin{itemize}
    ...
    \item A cancellation and overtime management mechanism based on a Newsvendor formulation that determines optimal time buffers for the last surgery in each session under duration uncertainty;
    \item A Mixed Integer Linear Programming (MILP) model for operating theatre allocation that incorporates practical constraints, including those associated with transplant surgeries, while remaining computationally tractable; and
    \item An empirical validation of the integrated framework in a large, resource-constrained public hospital, demonstrating its operational relevance and implementability.
\end{itemize}

By addressing those uncertainty factors within a unified framework, this work advances in the literature with an innovative integrated surgical planning and provides a practically grounded contribution for complex healthcare settings.